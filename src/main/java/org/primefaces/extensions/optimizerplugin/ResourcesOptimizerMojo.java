...
public class ResourcesOptimizerMojo extends AbstractMojo {
	...
	/**
	 * Files to be included. Files selectors follow patterns specified in
	 * {@link org.codehaus.plexus.util.DirectoryScanner}.
	 *
	 * @parameter
	 */
	...
	/**
	 * Files to be excluded. Files selectors follow patterns specified in
	 * {@link org.codehaus.plexus.util.DirectoryScanner}.
	 *
	 * @parameter
	 */
	...
	/**
	 * Default output directory for created source maps and original source files. This value is used internally in this
	 * class.
	 *
			...
			if (resourcesSets == null || resourcesSets.isEmpty()) {
				...
				for (ResourcesSet rs : resourcesSets) {
					...
					for (Aggregation aggr : aggrs) {
						...
						// evaluate inputDir
						File dir =
							(aggr != null && aggr.getInputDir() != null) ? aggr.getInputDir() : rs.getInputDir();
						if (dir == null) {
							dir = inputDir;
						}
						...
						if (aggr != null && aggr.getOutputFile() == null) {
							...
							if (files != null) {
								for (File file : files) {
									if (file.isDirectory()) {
										...
										if (!subDirCssFiles.isEmpty()) {
											DataUriTokenResolver dataUriTokenResolver =
												(useDataUri || rs.isUseDataUri() ? getDataUriTokenResolver()
													: null);

											// handle CSS files
											processCssFiles(file, subDirCssFiles, dataUriTokenResolver,
												getSubDirAggregation(file, aggr,
													ResourcesScanner.CSS_FILE_EXTENSION, suffix),
												null);
										}
										...
										if (!subDirJsFiles.isEmpty()) {
											...
											processJsFiles(file, subDirJsFiles,
												...
												resolveSourceMap(rs), null, resolveLanguageIn(rs),
												resolveLanguageOut(rs));
										}
									}
								}
							}
							...
							if (!scanner.getJsFiles().isEmpty()) {
								...
								processJsFiles(dir, scanner.getJsFiles(), aggr, resolveCompilationLevel(rs),
									resolveWarningLevel(rs), resolveSourceMap(rs), suffix, resolveLanguageIn(rs),
									resolveLanguageOut(rs));
							}
						}
					}
	...
	private void processJsFiles(File inputDir, Set<File> jsFiles, Aggregation aggr,
		CompilationLevel compilationLevel, WarningLevel warningLevel, SourceMap sourceMap, String suffix,
		LanguageMode languageIn, LanguageMode languageOut) throws MojoExecutionException {
		resFound = true;
		ResourcesSetAdapter rsa = new ResourcesSetJsAdapter(inputDir, jsFiles, aggr, compilationLevel, warningLevel,
			sourceMap, encoding, failOnWarning, suffix, languageIn, languageOut);
	...
	private LanguageMode getLanguageIn(String languageIn) throws MojoExecutionException {
		try {
			...
			final String errMsg = "Input language spec'" + languageIn
				+ "' is wrong. Valid constants are: 'ECMASCRIPT3', "
				+ "'ECMASCRIPT5','ECMASCRIPT5_STRICT','ECMASCRIPT6','ECMASCRIPT6_STRICT'," + "'ECMASCRIPT6_TYPED'";
			if (failOnWarning) {
				throw new MojoExecutionException(errMsg);
			} else {
				getLog().warn(errMsg);
